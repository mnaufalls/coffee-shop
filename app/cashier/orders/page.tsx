...
import type { OrderStatus } from "@/components/cashier/order-card";
...
export default function CashierOrdersPage() {
  ...
  const { orders, isLoading, error, totalPages, updatingId, fetchOrders, updateOrderStatus } =
    useOrders({ pageSize: PAGE_SIZE });
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      return (
        matchesStatus &&
        (orderId.includes(normalizedSearch) || customerName.includes(normalizedSearch))
      );
    });
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-8 p-6">
      <header>
        ...
        <p className="mt-2 text-sm text-zinc-600">Manage and process customer orders.</p>
      </header>
      ...
      <section className="flex items-center justify-between">
        <h2 className="text-2xl font-black">
          {filter === "all"
            ? "All Orders"
            : `${filters.find((item) => item.value === filter)?.label} Orders`}
        </h2>
