...
export default function CashierPage() {
  ...
  const summary = useMemo(() => {
    const todayOrders = orders.filter(
      (o) => getWIBDate(o.createdAt) === todayStr,
    );
    ...
    const totalRevenueOrders = todayOrders.filter(
      (o) => o.status !== "cancelled",
    );
    const avgPerOrder =
      totalRevenueOrders.length > 0
        ? todaySales / totalRevenueOrders.length
        : 0;

    const hourlyData: number[] = Array(24).fill(0);
    todayOrders.forEach((o) => {
      const h = getWIBHour(o.createdAt);
      hourlyData[h]++;
    });
    ...
    <main className="mx-auto max-w-7xl space-y-8 p-6">
      <header>
        ...
        <p className="mt-2 text-sm text-zinc-600">
          Monitor today&apos;s orders and sales (WIB).
        </p>
      </header>
        ...
        <div className="border-2 border-black bg-blue-300 p-6 shadow-[5px_5px_0_0_#000]">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-3 text-3xl font-black">
                {formatRupiah(summary.avgPerOrder)}
              </p>
            </div>
            <CurrencyDollar size={42} weight="bold" />
          </div>
          ...
          {summary.hourlyData.map((count, hour) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="w-full bg-orange-400 border border-black min-h-[2px] transition-all"
                  style={{
